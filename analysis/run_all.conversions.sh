...
WORK_DIR="${1:-.}"
IP_TO_GEO_ACCURACY_RADIUS=100
IGDB_INCLUDE_AS_LOCATIONS=0
GENERATE_ISO=0

export WORK_DIR
export IP_TO_GEO_ACCURACY_RADIUS
export IGDB_INCLUDE_AS_LOCATIONS
export GENERATE_ISO
...
./itdk_geo.py --convert-ip-to-latlon --remove-duplicate-consecutive-hops --filter-geo-coordinate-by-ground-truth --geo-coordinate-ground-truth-csv ./results/geo_distributions/geo_distribution.all.csv --maxmind-database ../data/maxmind/GeoLite2-City.mmdb --accuracy-radius $IP_TO_GEO_ACCURACY_RADIUS --routes_file region_pair.by_ip/routes.*.by_ip --output-dir "$WORK_DIR" --outputs
...
process_region_by_geo()
{
    ...
    igdb_client_args=()
    if [ $IGDB_INCLUDE_AS_LOCATIONS -ne 0 ]; then
...
for file in "$WORK_DIR"/routes.*.by_geo; do
    ...
    if [ -f "$name.by_geo.distribution" ]; then
        echo "Skipping $name.by_geo as it has been processed."
        continue
    fi
