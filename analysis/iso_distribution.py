...
# given a ip address, change it to coordinates
def convert_ip_to_coordinate(ip_addresses, node_ip_to_id, node_geo_df):
    # Convert each IP address to a node ID using the node_ip_to_id dictionary
    node_id = node_ip_to_id[ip_addresses]

    # Convert node IDs to coordinates using the node_geo_df dictionary
    if node_id not in node_geo_df.index:
        print(f'Node ID {node_id} not found in node_geo_df', file=sys.stderr)
        return None # None is returned for unknown Ip address
    ...
    for region, d_node_to_matches in d_by_region.items():
        ...
        for node_id in d_node_to_matches:
            for _, ip in d_node_to_matches[node_id]:
                ...
                if convert_ip_to_coordinate(ip, node_ip_to_id, node_geo_df):
                    coordinates.append(convert_ip_to_coordinate(
                        ip, node_ip_to_id, node_geo_df))
        coordinates_by_region[region] = coordinates
        print(f'Found {len(coordinates)} coordinates for {cloud}:{region}.', file=sys.stderr)
...
def get_iso_occurance_by_region(isos_by_region: dict[str, list[str]]) -> dict[str, dict[str, int]]:
    iso_occurance_by_region = {}
    for region, iso_list in isos_by_region.items():
        region_iso_occurance = {}
        for iso in iso_list:
            region_iso_occurance[iso] = region_iso_occurance.get(iso, 0) + 1
        iso_occurance_by_region[region] = region_iso_occurance
    return iso_occurance_by_region


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--cloud', required=True,
                        choices=['aws', 'gcloud'], help='The cloud provider you wish to get the iso occurance')
...
def main():
    ...
    iso_occurance_by_region = get_iso_occurance_by_region(isos_by_region)
    
    # print out the result or save to a file
    for region, iso_occurance in iso_occurance_by_region.items():
        print(f"Region {region} has iso distributions: {iso_occurance}")
